...
bool prenasal(void) {
	return last_prenasal == KC_I && timer_elapsed(prenasal_proximity) < MIN_PROXIMITY && !(get_mods() & ~MOD_MASK_SHIFT & MOD_MASK_CAG);
};

bool postvowel(void) {
	return last_vowel == KC_I && timer_elapsed(vowel_proximity) < MIN_PROXIMITY && !(get_mods() & ~MOD_MASK_SHIFT & MOD_MASK_CAG);
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	[0] = LAYOUT_ortho_5x15(/* Base */
			...
			KC_GRV, KC_COMM, KC_Z, KC_W, KC_Y, KC_X, KC_LCBR, KC_PSCR, KC_RCBR, KC_G, KC_P, KC_V, KC_N, KC_SCLN, KC_MINS,
			KC_TAB, KC_F, HM, KC_E, KC_A, KC_O, LCTL(KC_BSPC), MO(1), KC_BSPC, KC_D, KC_T, KC_SPC, KC_S, KC_C, KC_HOME,
			KC_PGUP, KC_DOT, KC_J, KC_M, KC_Q, KC_QUOT, KC_SLSH, KC_LGUI, KC_BSLS, KC_K, KC_B, KC_I, KC_H, KC_UP, KC_END,
			KC_PGDN, KC_CAPS, KC_INS, KC_DEL, KC_U, IN, OSM(MOD_LSFT), KC_RCTL, KC_RALT, KC_R, KC_L, KC_ENT, KC_LEFT, KC_DOWN, KC_RGHT
		),
