...
bool prenasal(void) {
	return last_prenasal == KC_A && timer_elapsed(prenasal_proximity) < MIN_PROXIMITY && !(get_mods() & ~MOD_MASK_SHIFT & MOD_MASK_CAG);
};

bool postvowel(void) {
	return last_vowel == KC_A && timer_elapsed(vowel_proximity) < MIN_PROXIMITY && !(get_mods() & ~MOD_MASK_SHIFT & MOD_MASK_CAG);
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	[0] = LAYOUT_ortho_5x15(/* Base */
			...
			KC_TAB, KC_F, HM, AN, KC_I, KC_U, LCTL(KC_BSPC), MO(1), KC_BSPC, KC_D, KC_T, KC_SPC, KC_S, KC_C, KC_HOME,
			KC_PGUP, KC_DOT, KC_Z, KC_A, KC_X, KC_QUOT, KC_SLSH, KC_LGUI, KC_BSLS, KC_K, KC_M, KC_Q, KC_H, KC_UP, KC_END,
			KC_PGDN, KC_CAPS, KC_INS, KC_DEL, KC_O, KC_E, OSM(MOD_LSFT), KC_RCTL, KC_RALT, KC_R, KC_L, KC_ENT, KC_LEFT, KC_DOWN, KC_RGHT
		),
