...
bool promote_consonants(void) {
	return predecessor_key == KC_I && timer_elapsed(vowel_proximity) < MIN_PROXIMITY && !(get_mods() & ~MOD_MASK_SHIFT & MOD_MASK_CAG);
};
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    if (keycode == IN) {
        ...
        if (record->event.pressed) {
			...
			keycode = promote_consonants()?KC_N:KC_I;	
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	[0] = LAYOUT_ortho_5x15(/* Base */
			...
			KC_QUOT, KC_Z, KC_I, KC_U, KC_F, KC_W, KC_LCBR, KC_PSCR, KC_RCBR, KC_G, KC_M, KC_P, KC_N, KC_J, KC_MINS,
			KC_TAB, KC_H, KC_O, KC_E, KC_A, KC_C, KC_LPRN, MO(1), KC_RPRN, KC_L, KC_T, KC_R, KC_S, KC_D, KC_ENT,
			KC_CAPS, KC_SCLN, KC_X, KC_Y, KC_COMM, KC_DOT, KC_SLSH, KC_LGUI, KC_BSLS, KC_K, KC_V, KC_B, RSFT_T(KC_Q), KC_UP, KC_INS,
			KC_ESC, KC_HOME, KC_END, KC_DEL, KC_BSPC, IN, KC_LSFT, KC_RCTL, KC_RALT, KC_SPC, KC_PGDN, KC_PGUP, KC_LEFT, KC_DOWN, KC_RGHT
		),
	[1] = LAYOUT_ortho_5x15(/* Function keys */
			...
			KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_TRNS, KC_NO, KC_NO, KC_NO, KC_NO, KC_TRNS, KC_NO, KC_NO,
